...
def mie(l_r, l_a, eps, sigma, r, rc=1000):
    '''
...
def mix(eps, sigma, rule="LB", k=0.0):
    ...
    """
    if (isinstance(sigma, (int, float, str, dict)) or 
        isinstance(eps, (int, float, str, dict))):
    ...
    eps = list(eps)
    if rule=="LB":
        return np.sqrt(eps[0]*eps[1]), np.mean(sigma), 
    else:
...
def mie_integral(l_r, l_a, eps, sigma, r_lower, r_upper):
    ...
    '''
    diff =  mie_int(l_r, l_a, eps, sigma, r_upper) - mie_int(l_r, l_a, eps, sigma, r_lower)
