...
class PlotSims():
    ...
    def plot_timeseries(self, ax, prop, blocksize=10, timeseries=False, 
                        bounds=None, color=None, alpha=1, label=None):
        """
    ...
    def plot_mean(self, ax, prop, bounds=None, drift=False, err=False, 
                  blocksize=15, color=None, alpha=1, label=None):        
        ...
        if err:
            std = np.std(blocks)
            ax.fill_between((x[0], x[-1]),(mean-std, mean-std) , 
                            (mean+std, mean+std),
    ...
    def write_stats(self, ofile="energies.out", 
                    props=True, blocks=10, bounds=None):
        stats = self.get_stats(props=props, blocks=blocks, bounds=bounds)
        with open(ofile, "w") as f:
            f.write("{:<18s}{:<12s}{:<12s}{:<12s}\n".format("Property", "Mean", "Error", "Drift"))
            for prop, mean, err, drift in zip(*stats):
                f.write("{:<18s}{:<12.4f}{:<12.4f}{:<12.4f}\n".format(prop, mean, err, drift))
        #Routine to write shizzle  
    
    def create_combined_property(self, props, name,average=True, difference=False, absolute_values=False):
...
class PlotLAMMPS(PlotSims):
    def get_properties(self):
        properties =  np.loadtxt(self.infile, comments=None, dtype=str, 
                                 skiprows=1, max_rows=1)
        for i, prop in enumerate(properties):
            prop = prop.strip("c_")
            prop = prop.strip("v_")
            if "press[1]" in prop: prop = prop.split("[1]")[0]+"_xx"
            if "press[2]" in prop: prop = prop.split("[2]")[0]+"_yy"
            if "press[3]" in prop: prop = prop.split("[3]")[0]+"_zz"
            if "press[4]" in prop: prop = prop.split("[4]")[0]+"_xy"
            if "press[5]" in prop: prop = prop.split("[5]")[0]+"_xz"
            if "press[6]" in prop: prop = prop.split("[6]")[0]+"_yz"
            properties[i] = prop
...
class ChunkData():
    ...
    def _chunksize_varies(self, chunks):
        self.binsize_varies = np.any(chunks 
                              - chunks[0]*np.ones(np.shape(chunks)))

    def _get_data(self, tmin, tmax):
        if tmin == self.tmin and tmax == self.tmax and isinstance(self.data, np.ndarray):
            return True
        ...
        if self.binsize_varies:
            ...
            for ti in range(tdiff):
                ...
                data[ti, :chunks, :chunks]= np.loadtxt(self.infile, 
            ...
            dchunks = chunks+1
            for ti in range(tdiff):
                data[ti, :, :]= np.loadtxt(self.infile, 
    ...
    def combine_quantity(self, data, tmin=None, tmax=None, 
        ...
        if method == "add":
            combined_quantity = np.zeros((len(self.data[:,0,0]), len(self.data[0,:,0])))
        if method == "mul":
            combined_quantity = np.ones((len(self.data[:,0,0]), len(self.data[0,:,0])))
        else:
            combined_quantity = self.get_data(data[0], tmin, tmax)
            data = data[1:]

        method_dict = {"add": np.add, "sub": np.subtract, 
                       "div": np.divide, "mul": np.multiply
                      }
        for item in data:
    ...
    def get_stats(self, data, x0, xE, tmin=None, tmax=None):
        ...
        if bound_cross:
            ydata = np.concatenate((ydata[:, :index0],
                                   ydata[:, indexE:]), axis=1)
