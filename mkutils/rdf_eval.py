class RDFEval():
    ...
    def _get_r0r1(self):
        ...
        i = 1 
        while self.rdf[ind0[0]+i+1] < self.rdf[ind0[0]+i]: i +=1
        ...
        i = 1
        while self.rdf[indv0[0]+i+1] > self.rdf[indv0[0]+i]: i += 1
        ...
        i = 1
        while self.rdf[ind1[0]+i+1] < self.rdf[ind1[0]+i]: i +=1
    ...
    def _integrate(self, lower_bound, upper_bound):
        x2 = np.multiply(self.bins[lower_bound:upper_bound+1],
    ...
    def print_results(self):
        ...
        """
        if None in [self.min0, self.min1, 
                    self.max0, self.max1,
                    self.n0  , self.n1
                   ]:
            self.get_n0n1()
        print("Rmax-1st:   {:.4f} nm".format(self.bins[self.max0]/10.))
        print("Rmin-1st:   {:.4f} nm".format(self.bins[self.min0]/10.))
        print("Coord#-1st: {:.4f}\n".format(self.n0))
        print("Rmax-2nd:   {:.4f} nm".format(self.bins[self.max1]/10.))
        print("Rmin-2nd:   {:.4f} nm".format(self.bins[self.min1]/10.))
        print("Coord#-2nd: {:.4f}\n".format(self.n1))
    ...
    def plot(self, ax):
        ...
        ax.fill_between(self.bins[:self.min0+1], 
                        self.rdf[:self.min0+1], 
                        np.zeros(self.rdf[:self.min0+1].shape),
                        color="k", alpha=0.7)
        ax.fill_between(self.bins[self.min0:self.min1+1], 
