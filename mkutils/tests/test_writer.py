...
this_ff = FFWriter("test.json")

this_ff.add_atomtype("W", 8, 6, 305.21, 0.29016, 18.015028)

this_ff.add_atomtype("T", 15.947, 6, 358.37, 0.45012, 43.08698)

this_ff.add_atomtype("M", 16.433, 6, 377.14, 0.4184, 42.07914)

this_ff.add_crossint("W", "T", 0.7)

this_ff.add_atomtype("Na", 8, 6, 50, 0.23, 23.0)
...
def test_atomtype1():
    with pytest.raises(ValueError, match=r"Atomtype already .*"):
        this_ff.add_atomtype("Na", 8, 6, 55, 0.24, 23.1)


def test_atomtype2():
    this_ff.add_atomtype("Na", 9, 7, 55, 0.24, 23.1, update=True)
    with open("test.json", "r") as f:
        _ff_dict = json.load(f)
