...
def create_fig(sp_row, sp_col, fig_width=8, fig_height=6, axes_label_size=24,
               axes_tick_size=20, legend_fontsize=16, marker_size=6, line_width=1):
    ...
    fig1 = plt.figure(num=None, figsize=fig_size, dpi=fig_dpi, 
                      facecolor=fig_facecolor, edgecolor=fig_edgecolor, 
                      linewidth=fig_edgelinewidth, frameon=fig_frameon,
    ...
    for i in range(sp_row*sp_col):
        ...
        newax = fig1.add_subplot(sp_row, sp_col,i+1, xscale=splot_xscale,
                    yscale=splot_yscale)
...
def set_ticks(x_major, x_minor, y_major, y_minor, 
              x_major_format, y_major_format, ax):
    ...
    return (ax.xaxis.set_major_locator(xmajorLocator), 
...
def get_color(i):
    color_list = ['#000000', '#4477AA', '#66CCEE', '#228833', 
...
def labelLine(ax,line_num,xy,label,y_adjust):
    ...
    return ax.text(xy[0], xy[1], label, rotation=trans_angle, fontsize=10, 
                   ha='center', va='center', zorder=2.5, 
